...
{
    [BepInPlugin("17N.Eyjafjalla", "EyjafjallaMod", "0.1.0")]
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        {
                ModelReplacementAPI.RegisterModelReplacementOverride(typeof(MREYJAFJALLA));

            }
            if (enableModelAsDefault.Value)
            {
                ModelReplacementAPI.RegisterModelReplacementDefault(typeof(MREYJAFJALLA));

            }
            var commaSepList = suitNamesToEnableModel.Value.Split(',');
            foreach (var item in commaSepList)
            {
                ModelReplacementAPI.RegisterSuitModelReplacement(item, typeof(MREYJAFJALLA));
            }
                

            Harmony harmony = new Harmony("17N.Eyjafjalla");
            harmony.PatchAll();
            Logger.LogInfo($"Plugin {"17N.Eyjafjalla"} is loaded!");
        }
    }
    public static class Assets
    {
        // Replace mbundle with the Asset Bundle Name from your unity project 
        public static string mainAssetBundleName = "Eyjafjalla";
